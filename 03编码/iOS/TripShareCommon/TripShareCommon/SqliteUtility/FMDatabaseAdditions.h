...
/*执行操作，增，删，改，查，所有操作。
 不需要open,close,
 这里已经封装了open,close,只需要在这里写入代码块。 
 
 __block FMResultSet *value;
 [self executeDataBaseOPeration:^{
 value = [self executeQuery:@""];
 }];
 
 [value resultDictionary];
 
 */
- (void)executeDataBaseOPeration:(void (^)(void))block;
